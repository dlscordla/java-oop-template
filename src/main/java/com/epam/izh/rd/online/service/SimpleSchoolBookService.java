...
public class SimpleSchoolBookService implements BookService<SchoolBook> {
    ...
    public Author findAuthorByBookName(String name) {
        SchoolBook schoolBooksDatabase = schoolBookBookRepository.findByName(name)[0];
        if (authorService.count() != 0) {
            return authorService.findByFullName(schoolBooksDatabase.getAuthorName(), schoolBooksDatabase.getAuthorLastName());
        } else {
            return null;
        }
    }
